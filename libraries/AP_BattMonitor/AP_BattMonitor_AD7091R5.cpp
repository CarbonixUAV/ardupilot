...
void AP_BattMonitor_AD7091R5::init()
{
    ...
    if (_volt_pin.get() >= AD7091R5_BASE_PIN && _volt_pin.get() <= AD7091R5_BASE_PIN + AD7091R5_NO_OF_CHANNELS) {
        ...
        if (_curr_pin.get() >= AD7091R5_BASE_PIN && _curr_pin.get() <= AD7091R5_BASE_PIN + AD7091R5_NO_OF_CHANNELS) {
            curr_buff_pt = _curr_pin.get() - AD7091R5_BASE_PIN;
            _state.healthy = true; //set status healthy
        }
    }

    //return if configured incorrectly
    if (!_state.healthy) {
        return;
    }
