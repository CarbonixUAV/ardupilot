...
#include "AP_Can_node_stats.h"
#include <AP_RTC/AP_RTC.h>

// table of user settable parameters
const AP_Param::GroupInfo AP_Can_node_stats::var_info[] = {
    ...
    // @ReadOnly: True
    // @User: Standard
    AP_GROUPINFO("_RUNTIME",    2, AP_Can_node_stats, params.runtime, 0),
    ...
    // @ReadOnly: True
    // @User: Standard
    AP_GROUPINFO("_RESET",    3, AP_Can_node_stats, params.reset, 1),

    AP_GROUPEND
};

AP_Can_node_stats *AP_Can_node_stats::_singleton;

// constructor
AP_Can_node_stats::AP_Can_node_stats(void)
{
    _singleton = this;
}
...
void AP_Can_node_stats::set_flying(const bool is_flying)
{
    if (is_flying) {
        if (!_flying_ms) {
...
{
    return AP_Can_node_stats::get_singleton();
}